...
Calculates year-over-year percentage change for historical and projected data
...
for country in countries:    
    ...
    # Calculate growth rates for historical data (wup_urban_pop and wup_rural_pop)
    for aoi in ['urban', 'rural']:
        col_name = f'wup_{aoi}_pop'
        if col_name in country_pivot.columns:
            growth_rate_col = f'{aoi}_growth_rate'
            country_pivot[growth_rate_col] = country_pivot[col_name].pct_change() * 100
    ...
    for aoi in ['urban', 'rural']:
        ...
        if median_col in country_pivot.columns:
            ...
            country_pivot[growth_rate_col] = country_pivot[median_col].pct_change() * 100
