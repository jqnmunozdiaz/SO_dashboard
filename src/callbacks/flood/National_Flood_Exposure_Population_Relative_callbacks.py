...
try:
    from ...utils.flood_data_loader import load_flood_exposure_data, load_ghsl_total_population_data, filter_flood_data
    ...
    from src.utils.flood_data_loader import load_flood_exposure_data, load_ghsl_total_population_data, filter_flood_data
...
def register_national_flood_exposure_population_relative_callbacks(app):
    """Register callbacks for National Flood Exposure (Population, Relative) chart"""
    
    # Load static data once at registration time for performance
    flood_data = load_flood_exposure_data('pop')
    total_population_data = load_ghsl_total_population_data()
    ...
    def generate_national_flood_exposure_population_relative_chart(selected_country, selected_flood_type, selected_return_periods, combined_benchmarks):
        ...
        try:
            ...
            # Merge flood data with total population data
            ...
            merged_data['relative_exposure_pct'] = (
                merged_data['ftm3_ghsl_total_pop_#'] / 
                merged_data['ghsl_total_pop_#']
            ) * 100
